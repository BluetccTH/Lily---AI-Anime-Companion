...
export default function App() {
  ...
  const handleSendMessage = async (userText: string, attachment?: ChatAttachment) => {
    ...
    try {
      // Direct high-performance Cloud Gemini Engine with vision and long-term memory
      ...
      const data = await res.json();
      const replyText = data.reply || 'ลิลลี่อยู่เคียงข้างคุณใต้ฟากฟ้าแห่งดวงดาวเสมอค่ะ ✨';
      ...
      const modelMsg: ChatMessage = {
        id: `model-${Date.now()}`,
        role: 'model',
        text: replyText || 'ลิลลี่อยู่เคียงข้างคุณใต้ฟากฟ้าแห่งดวงดาวเสมอค่ะ ✨',
        expression,
        action,
        timestamp: Date.now(),
      };
  ...
  return (
    <main className="relative w-screen h-screen overflow-hidden bg-slate-950 flex flex-col items-center justify-center select-none">
      ...
      <section className="relative z-10 w-full h-full flex items-center justify-center">
        <Live2DCanvas
          modelId={settings.characterModel || 'haru'}
          currentExpression={currentExpression}
